...
export default function SidebarItem(props: SidebarItemProps): JSX.Element {
  return (
    <div className={`pr-6 h-12`}>
      <button className={`flex pl-8 w-full h-full items-center rounded-r-full gap-4 ${props.checked && 'bg-mainPurple cursor-default'} ${!props.checked && 'pr-6 transition outline-none hover-duration-150 ease-in-out hover:scale-105 active:scale-90 select-none'}`}>
