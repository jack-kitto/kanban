...
export default function PopoverMenu(props: PopoverMenuProps): JSX.Element {
  ...
  return (
    <div className="flex justify-end items-end">
      <Popover isOpen={isOpen} onOpenChange={(open) => setIsOpen(open)} placement="left-start" showArrow={true}>
        <PopoverTrigger>
          ...
          <div className={`flex flex-col px-[17px] py-5 min-w-[192px] items-start`}>
            {props.options.map((option: PopoverMenuOption, index: number): JSX.Element => (
              <Button
                ...
                className={`bg-white w-full justify-start dark:bg-veryDarkGray p-2 select-none prose-bl ${option.destructive ? 'text-red' : 'text-mediumGray dark:text-white'}`}
              >
                {option.text}
              </Button>
            ))}
