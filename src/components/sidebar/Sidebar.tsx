...
const SidebarWrapper = observer(({ children }: { children: React.ReactNode }) => {
  ...
  if (theme.darkMode) return (
    <div key={"darkmodesidebar"} className='flex flex-col h-full w-1/6 border-r-1 border-linesDark bg-darkGrey'>
      {children}
    </div>
  )
  return (
    <div key={"lightmodesidebar"} className='flex flex-col h-full w-1/6 border-r-1 border-linesLight bg-white'>
      {children}
    </div>
  )
})
